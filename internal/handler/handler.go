...
func handle(ctx context.Context, cfg *config.Config, payload []byte, deps Dependencies) error {
	...
	if err != nil {
		return fmt.Errorf("handler: parse event: %w", err)
	}

	if evt.SecretARN != cfg.SecretARN {
		return fmt.Errorf("handler: event secret ARN %q does not match configured %q", evt.SecretARN, cfg.SecretARN)
	}
	...
	if cfg.TargetSecretARN != "" {
		...
		if err != nil {
			return fmt.Errorf("handler: get secret: %w", err)
		}
	...
	if cfg.ECSForceDeploy {
		if err := deps.ECS.ForceNewDeployment(ctx, cfg.ECSCluster, cfg.ECSServices); err != nil {
			return fmt.Errorf("handler: ecs redeploy: %w", err)
		}
	}
